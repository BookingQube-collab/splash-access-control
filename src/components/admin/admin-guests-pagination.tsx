...
import { buildPageList } from "@/components/admin/admin-guests-utils";
import { cn } from "@/lib/utils";

export function AdminGuestsPagination({
  page,
  pageSize,
  total,
  onPageChange,
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-[#f1f5f9] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <p className="text-sm text-[#64748b]">
        ...
        <span className="font-semibold text-[#334155]">{total}</span> guest
        {total === 1 ? "" : "s"}
      </p>
